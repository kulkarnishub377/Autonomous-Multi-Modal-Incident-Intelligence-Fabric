...
from app.schemas import OmniSightEventIn, EventOut, TemperatureSensorRequest, VisionMockRequest
...
def ingest_event(payload: OmniSightEventIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def mock_vision(payload: VisionMockRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    event = event_service.ingest(db, OmniSightEventIn(
